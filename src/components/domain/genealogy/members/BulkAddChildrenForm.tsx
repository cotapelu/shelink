...
function SpouseSelect({ spouses, selectedSpouseId, setSelectedSpouseId }: { spouses: SpouseOption[]; selectedSpouseId: string; setSelectedSpouseId: (id: string) => void }) {
  return (
    <div>
      ...
      <select value={selectedSpouseId} onChange={e => setSelectedSpouseId(e.target.value)} className="flex-1 bg-white text-stone-900 placeholder-stone-400 text-sm rounded-md sm:rounded-lg border-stone-300 shadow-sm focus:border-sky-500 focus:ring-sky-500 p-2 sm:p-2.5 border transition-colors">
        ...
        {spouses.map(rel => <option key={rel.id} value={rel.id}>{rel.full_name} {rel.note ? `(${rel.note})` : ""}</option>)}
      </select>
    </div>
  );
}

function ChildRow({ child, index, onChange, onRemove }: { child: BulkChild; index: number; onChange: (i: number, f: keyof BulkChild, v: string) => void; onRemove: (i: number) => void }) {
  return (
...
function ChildList({ bulkChildren, setBulkChildren }: { bulkChildren: BulkChild[]; setBulkChildren: (c: BulkChild[]) => void }) {
  const handleChange = (i: number, f: keyof BulkChild, v: string) => {
    const nb = [...bulkChildren];
    nb[i] = { ...nb[i], [f]: v };
    setBulkChildren(nb);
  };
  const handleRemove = (i: number) => {
    const nb = bulkChildren.filter((_, idx) => idx !== i);
    if (nb.length === 0) nb.push({ name: "", gender: "male", birthYear: "", isProcessing: false });
    setBulkChildren(nb);
  };
  return (
    <div className="space-y-2">
      <label className="block text-xs font-medium text-stone-500 mb-1">Danh sách các con</label>
      {bulkChildren.map((child, index) => <ChildRow key={index} index={index} child={child} onChange={handleChange} onRemove={handleRemove} />)}
...
function FormButtons({ onSave, onCancel, processing, disabled }: { onSave: () => Promise<void>; onCancel: () => void; processing: boolean; disabled: boolean }) {
  return (
...
export default function BulkAddChildrenForm({ spouses, selectedSpouseId, setSelectedSpouseId, bulkChildren, setBulkChildren, processing, onSave, onCancel }: Props) {
  return (
