...
function NoteField({ note, setNote }: { note: string; setNote: (s: string) => void }) {
  return (
    <div>
      ...
      <input type="text" placeholder="VD: Vợ cả, Vợ hai, Chồng trước..." value={note} onChange={e => setNote(e.target.value)} className="bg-white text-stone-900 placeholder-stone-400 block w-full text-sm rounded-md sm:rounded-lg border-stone-300 shadow-sm focus:border-amber-500 focus:ring-amber-500 p-2 sm:p-2.5 border mb-3 transition-colors" />
    </div>
  );
}
...
function DirectionSelect({ direction, setDirection }: { direction: "parent" | "child" | "spouse"; setDirection: (d: "parent" | "child" | "spouse") => void }) {
  return (
    <div>
      ...
      <select value={direction} onChange={e => setDirection(e.target.value as "parent" | "child" | "spouse")} className="bg-white text-stone-900 placeholder-stone-400 block w-full text-sm rounded-md sm:rounded-lg border-stone-300 shadow-sm focus:border-amber-500 focus:ring-amber-500 p-2 sm:p-2.5 border transition-colors">
...
function ChildTypeSelect({ type, setType }: { type: RelationshipType; setType: (t: RelationshipType) => void }) {
  return (
    <div>
      ...
      <select value={type} onChange={e => setType(e.target.value as RelationshipType)} className="bg-white text-stone-900 placeholder-stone-400 block w-full text-sm rounded-md sm:rounded-lg border-stone-300 shadow-sm focus:border-amber-500 focus:ring-amber-500 p-2 sm:p-2.5 border transition-colors">
...
function SearchResultItem({ p, onSelect }: { p: Person; onSelect: (p: Person) => void }) {
  return (
    <button onClick={() => onSelect(p)} className="px-3 py-2 hover:bg-amber-50 text-sm flex items-center justify-between border-b border-stone-100 last:border-0">
      <div className="flex items-center gap-2">
        <span className={`flex items-center justify-center text-[8px] font-bold size-3 rounded-full text-white shrink-0 ${p.gender === "male" ? "bg-sky-500" : p.gender === "female" ? "bg-rose-500" : "bg-stone-400"}`}>
          {p.gender === "male" ? "♂" : p.gender === "female" ? "♀" : "?"}
        </span>
        <span className="font-medium text-stone-800">{p.full_name}</span>
      </div>
      <span className="text-[10px] text-stone-400">{formatDisplayDate(p.birth_year, p.birth_month, p.birth_day)}</span>
    </button>
  );
}
...
function SearchResults({
  ...
  return (
    <div className="mt-2 bg-white border border-stone-200 rounded-md shadow-lg max-h-[250px] overflow-y-auto">
      ...
      {list.map(p => (
        <SearchResultItem key={p.id} p={p} onSelect={onSelect} />
      ))}
    </div>
  );
}
...
function FormButtons({
  ...
  return (
    <div className="flex gap-2 pt-2">
      <button onClick={onSave} disabled={disabled || processing} className="flex-1 bg-amber-700 text-white py-2 sm:py-2.5 rounded-md sm:rounded-lg text-sm font-medium hover:bg-amber-800 disabled:opacity-50 transition-colors">{processing ? "Đang lưu..." : "Lưu"}</button>
...
function FormFields({ newRelNote, setNewRelNote, newRelDirection, setNewRelDirection, newRelType, setNewRelType }: FormFieldsProps) {
  return (
    <>
      <NoteField note={newRelNote} setNote={setNewRelNote} />
      <DirectionSelect direction={newRelDirection} setDirection={setNewRelDirection} />
      {(newRelDirection === "child" || newRelDirection === "parent") && (
        <ChildTypeSelect type={newRelType} setType={setNewRelType} />
      )}
    </>
  );
}
...
function FormContent(props: FormUIProps) {
  const { newRelNote, setNewRelNote, newRelDirection, setNewRelDirection, newRelType, setNewRelType, searchTerm, setSearchTerm, searchResults, setSearchResults, recentMembers, selectedTargetId, setSelectedTargetId, onSave, onCancel, processing, showDropdown, localError } = props; return (
...
export default function AddRelationshipForm(props: Props) {
  ...
  const showDropdown = props.searchResults.length > 0 || (props.searchTerm.length === 0 && !props.selectedTargetId && props.recentMembers.length > 0);
  const handleSave = async () => {
    if (!props.selectedTargetId) {
      setLocalError("Vui lòng chọn người liên quan");
      return;
    }
    setLocalError(null);
    await props.addRelationship();
    props.setSearchTerm("");
    props.setSelectedTargetId("");
    props.setNewRelNote("");
  };
  return (
    <FormUI
