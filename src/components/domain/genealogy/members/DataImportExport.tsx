...
export default function DataImportExport() {
    ...
    async function fetchPersons() {
      try {
        ...
        const result = await api.get<Person[]>(API_ENDPOINTS.PERSONS_LIST, {
          params: { order: "birth_year.asc", nullsFirst: false },
        });
  ...
  const handleExport = async (format: "json" | "gedcom") => {
    try {
      ...
      const data = await exportData(rootParam) as any;
      ...
      // CSV export disabled due to build issues
      // if (format === "csv") { ... }
  ...
  const handleConfirmImport = async () => {
    ...
    try {
      ...
      }) as { error?: string; imported?: { persons: number; relationships: number } };
