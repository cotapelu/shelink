...
export default function AdminUserList({
  ...
  const handleRoleChange = async (userId: string, newRole: UserRole) => {
    ...
    try {
      setLoadingId(userId);
      const result = await changeUserRole(userId, newRole) as { error?: string } | undefined;
  ...
  const handleStatusChange = async (userId: string, newStatus: boolean) => {
    ...
    try {
      setLoadingId(userId);
      const result = await toggleUserStatus(userId) as { error?: string } | undefined;
  ...
  const handleDelete = async (userId: string) => {
    ...
    try {
      setLoadingId(userId);
      const result = await deleteUser(userId) as { error?: string } | undefined;
  ...
  const handleCreateUser = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const result = await adminCreateUser(formData) as { error?: string } | undefined;
          ...
          <div className="bg-white/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-stone-200/60 w-full max-w-md overflow-hidden transform transition-all">
            ...
            <form onSubmit={handleCreateUser} className="p-6">
              <div className="space-y-4">
                ...
                <div>
                  <label htmlFor="is_active" className="block text-sm font-medium text-stone-700 mb-1">
                    Trạng thái
                  </label>
                  <select
                    id="is_active"
