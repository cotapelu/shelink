...
try {
    ...
    if (isEditing && personId) {
      const updateResult = await api.patch(
        API_ENDPOINTS.PERSONS_UPDATE(personId),
        personData,
      );
      if (updateResult.error) throw new Error(updateResult.error);
    } else {
      const createResult = await api.post<{ id: string }>(
    ...
    if (isAdmin && personId) {
      ...
      const privateResult = await api.post("/person-details-private", privateData);
      if (privateResult.error) throw new Error(privateResult.error);
    }
